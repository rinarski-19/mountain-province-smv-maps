...
const SADANGA_CONFIG = {
  ...
  tiles: {
    ...
    // Sadanga is the per-building override LGU. PAO staff need
    // accurate rooftop imagery + recognised street names to verify
    // building identity, so we default the basemap to Google Streets
    // when the page loads on /?m=sadanga. Users can still switch via
    // the gear-icon tile mode picker; the default just sets the
    // initial value.
    defaultTileMode: "google_street",
  },
  ui: {
