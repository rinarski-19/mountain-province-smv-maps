...
import { resolvePrintLabels } from "./print-labels.js";
import {
  MAX_PRINT_ZOOM,
  CLASS_LABEL_MAX_MM,
  CLASS_LABEL_MIN_MM,
...
import { addedClassValues } from "./added-classes.js";
import { readAddedClasses } from "./added-classes-store.js";
import { getMunicipalityConfig } from "./municipalities.js";
import { readPalette } from "./palette-store.js";
import { PRINT_THEME_DEFAULTS, resolvePrintTheme } from "./print-theme.js";
import { readPrintSettings } from "./print-settings-store.js";
...
export { MIN_PRINT_ZOOM, MAX_PRINT_ZOOM, clampPrintZoom, clampPrintPan };
export { clampClassLabelScale };
...
// Populated from lib/print-theme.js before each build. Deliberately a
// mutable object rather than a frozen literal: every PALETTE.x read in
// this file then picks up an override with no other change. Safe because
// buildSvg is fully synchronous, so no request can interleave with
// another's theme.
const PALETTE = { ...PRINT_THEME_DEFAULTS };
...
function makeProjection(
  ...
  // Scale about the frame's centre so the subject stays put as the zoom
  // changes, rather than drifting toward a corner.
  const z = clampPrintZoom(zoom);
  drawW *= z;
  drawH *= z;
  // Centred first, then shifted. Pan is a fraction of the frame, so the
  // same value means the same thing whatever the paper size.
  const offsetX =
    frame.x + (usableW - drawW) / 2 + clampPrintPan(panX) * usableW;
  const offsetY =
    frame.y + (usableH - drawH) / 2 + clampPrintPan(panY) * usableH;
...
// Row metrics for the legend block. Shared so the footer reserved for the
// legend and the legend actually drawn can never disagree — they did: the
// reserve was a hardcoded 62 mm on landscape while the block is never
// shorter than ~97 mm, so the legend covered the map on every landscape
// barangay sheet.
export const LEGEND_METRICS = Object.freeze({
  padMm: 4,
  titleHeightMm: 9,
  infoLineHeightMm: 4.5,
  legendLabelHeightMm: 5,
  colHeaderHeightMm: 5,
  smvRowHeightMm: 4.6,
  roadRowHeightMm: 5.6,
  landmarkRowHeightMm: 5.2,
  roadRowCount: 4,
});

// Width of the title block's label column. Fixed at 28 mm it clipped:
// "BOUNDARY SHOWN:" needs ~30 mm at 3.2 pt, and since every caption in
// that block is user-editable, any long one collided with its value.
const INFO_LABEL_FONT_MM = 3.2;
// Bold uppercase Helvetica runs wider than the 0.62 factor used for mixed-
// case body text; at 0.62 "BOUNDARY SHOWN:" still touched its value.
const INFO_LABEL_CHAR_W = 0.74;
const INFO_LABEL_GAP_MM = 3;
export function infoLabelColumnMm(labels = []) {
  const longest = labels.reduce((n, t) => Math.max(n, String(t ?? "").length), 0);
  return Math.max(28, longest * INFO_LABEL_FONT_MM * INFO_LABEL_CHAR_W + INFO_LABEL_GAP_MM);
}

// Mirrors the column geometry in the legend drawing code below.
export function legendBlockWidthMm(infoLabels = []) {
  const M = LEGEND_METRICS;
  const innerGapMm = 1.2;
  const smvColWidth = 5 + innerGapMm + 7 + innerGapMm + 15;
  const roadColWidth = 7 + innerGapMm + 14;
  const landmarkColWidth = 5 + innerGapMm + 20;
  const colGapMm = 5;
  const columnsWidth =
    smvColWidth + colGapMm + smvColWidth + colGapMm + roadColWidth + colGapMm + landmarkColWidth;
  // The title block can be wider than the four columns once a caption is
  // long; the box has to hold whichever is bigger.
  const infoWidth = infoLabelColumnMm(infoLabels) + 34;
  return M.padMm * 2 + Math.max(columnsWidth, infoWidth);
}

export function legendBlockHeightMm({
  commercialCount = 0,
  residentialCount = 0,
  institutionalCount = 0,
  landmarkCount = 0,
  hasBarangayTarget = false,
  extraInfoRows = 0,
}) {
  const M = LEGEND_METRICS;
  const colHeightMm = Math.max(
    commercialCount * M.smvRowHeightMm,
    residentialCount * M.smvRowHeightMm,
    M.roadRowCount * M.roadRowHeightMm,
    landmarkCount * M.landmarkRowHeightMm
  );
  return (
    M.padMm * 2 +
    M.titleHeightMm +
    ((hasBarangayTarget ? 4 : 3) + extraInfoRows) * M.infoLineHeightMm +
    M.legendLabelHeightMm +
    institutionalCount * M.smvRowHeightMm +
    M.colHeaderHeightMm +
    colHeightMm
  );
}

export function printMapFitFrame(
  paperW,
  paperH,
  printOrientation,
  hasBarangayTarget,
  ...
  // Landscape: put the legend BESIDE the map, not under it.
  //
  // A3 landscape is only 297 mm tall and the legend is never shorter than
  // ~97 mm, so reserving a full-width footer for it cost a third of the
  // sheet height while leaving the bottom-left corner empty — the legend
  // is only ~130 mm wide on a 420 mm page. Taking a column on the right
  // instead gives the barangay the full sheet height.
  //
  // The column is full height while the legend fills only its lower part,
  // so part of it is blank — the signature block moves in there to use it.
  // Whether landscape or portrait yields the larger map is shape-dependent,
  // so both stay available.
  if (printOrientation === "landscape" && legendWidthMm > 0) {
    const topInset = PAGE_MARGIN_MM + 2;
    const columnW = legendWidthMm + PAGE_MARGIN_MM * 2;
    return {
      x: sideInset,
      y: topInset,
      width: paperW - sideInset - columnW,
      height: paperH - topInset - PAGE_MARGIN_MM - 1.5,
    };
  }
  ...
  //
  // The reserve is the legend's own measured height plus a gap, not a
  // constant: the block grows with the class ladder, and anything fixed
  // either clips the legend or wastes paper.
  const footerReserve = Math.ceil(legendHeightMm + PAGE_MARGIN_MM * 2);
...
export function buildSvg({
  ...
  barangayName = null,
  // Editable field names for the sheet. Overrides only; anything not
  // supplied falls back to PRINT_LABEL_DEFAULTS. See lib/print-labels.js.
  labels: labelOverrides = null,
  // Classes the municipality has withdrawn from its own schedule. Their
  // zones still paint (the geometry is real and on the ground), but the
  // legend stops advertising the class as a current tier.
  removedClasses = [],
  // See clampClassLabelScale. 1 is the class-code size every sheet used
  // before this became adjustable.
  classLabelScale = 1,
  // See clampPrintZoom. 1 fits the subject to the page.
  zoom = 1,
  // See clampPrintPan. 0/0 keeps the subject centred.
  panX = 0,
  panY = 0,
  // Set when the sheet is drawn on a parent barangay's outline because PSA
  // does not map this sitio separately. The title block names the sitio
  // the user asked for, with a line saying whose boundary is shown.
  barangayDisplayName = null,
  barangayBorrowedFrom = null,
}) {
  if (!outline) throw new Error(`Missing outline for ${slug}`);
  const L = resolvePrintLabels(labelOverrides);
  ...
  // Which classes the legend will list. Derived here, before projection,
  // so the footer can be sized to the block that is actually coming.
  const previewClasses = Array.from(
    new Set([
      ...Object.keys(classValues),
      ...(zones?.features ?? [])
        .map((f) => String(f?.properties?.classification ?? "").toUpperCase())
        .filter(Boolean),
    ])
  ).filter(
    (k) =>
      k !== "UNCLASSIFIED" &&
      !(removedClasses ?? []).some((r) => String(r).toUpperCase() === k)
  );
  // The BOUNDARY SHOWN: row on a sitio sheet is a real extra line. Without
  // it the box kept its 5-row height while drawing 6, so every row shifted
  // down and the last one sat on the border.
  const hasBorrowedBoundaryRow = Boolean(barangayBorrowedFrom);
  const reservedLegendHeightMm = legendBlockHeightMm({
    commercialCount: previewClasses.filter((k) => isCommercialClass(k)).length,
    residentialCount: previewClasses.filter((k) => isResidentialClass(k)).length,
    institutionalCount: previewClasses.includes("INSTITUTIONAL") ? 1 : 0,
    landmarkCount: LANDMARK_KIND_OPTIONS.length,
    hasBarangayTarget: Boolean(targetBarangayFeature),
    extraInfoRows: hasBorrowedBoundaryRow ? 1 : 0,
  });
  ...
  const proj = makeProjection(
    ...
    printMapFitFrame(
      ...
      paperH,
      printOrientation,
      Boolean(targetBarangayFeature),
      reservedLegendHeightMm,
      legendBlockWidthMm(
        (targetBarangayFeature
          ? [L.barangayLabel, ...(barangayBorrowedFrom ? ["BOUNDARY SHOWN:"] : [])]
          : []
        ).concat([L.municipalityLabel, L.provinceLabel, L.islandLabel])
      )
    ),
    zoom,
    panX,
  ...
  lines.push(
    `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" ` +
      ...
      (targetBarangayFeature
        ...
        : "") +
      ` data-zoom="${fmt(clampPrintZoom(zoom))}"` +
      ` data-pan="${fmt(clampPrintPan(panX))},${fmt(clampPrintPan(panY))}"` +
      ` data-label-scale="${fmt(clampClassLabelScale(classLabelScale))}"` +
  ...
  if (zoneLabelEntries.length) {
    ...
    // Not named labelScale: that identifier already means the landmark
    // label size elsewhere in this file.
    const classScale = clampClassLabelScale(classLabelScale);
    const classLabelMaxMm = CLASS_LABEL_MAX_MM * classScale;
    const classLabelMinMm = CLASS_LABEL_MIN_MM * classScale;
    ...
    for (const { feat, klass, bounds, drawIndex } of labelCandidates) {
      try {
        ...
        for (const tryVertical of [false, true]) {
          ...
          if (maxFontSize < classLabelMinMm) continue;
          ...
          ).filter((size) => size >= classLabelMinMm);
    ...
    for (const klass of new Set(zoneLabelEntries.map((entry) => entry.klass))) {
      ...
      for (const tryVertical of [false, true]) {
        for (const fontSize of [
          1.4, 1.2, 1.0, 0.9, 0.8, 0.75, 0.65, 0.55,
        ].map((size) => size * classScale)) {
        ...
        for (const fallbackEntry of fallbackEntries) {
          for (const candidate of zoneLabelPointCandidates(fallbackEntry.feat)) {
            try {
              ...
              const fontSize = CLASS_LABEL_MIN_MM * classScale;
    ...
    for (const { feat, klass, bounds, drawIndex } of labelCandidates) {
      ...
      // Scaled as a whole, not just the cap: fallbackTarget is derived
      // from the strip's own geometry, so capping first and scaling
      // after would leave every geometry-bound strip — most of them —
      // ignoring the setting.
      const baseFontSize =
        Math.max(0.58, Math.min(fallbackCap, fallbackTarget)) * classScale;
      const fontSizes = Array.from(
        new Set(
          [
            baseFontSize,
      ...
      ).filter((size) => size >= 0.48 * classScale);
        ...
        for (const vertical of orientations) {
          for (const fontSize of fontSizes) {
            ...
            const textHeight = vertical
              ...
              : fontSize * 0.95;
            const pad =
              fontSize < 0.65 * classScale ? 0.18 : SMV_LABEL_COLLISION_PAD_MM;
  ...
  if (showLocationLegend && locationLegendRows.length) {
    ...
    lines.push(
      `<text x="${fmt(locX + locPad)}" y="${fmt(locCursorY)}" ` +
        `font-size="3.1" font-weight="700" fill="#000000">` +
        escapeXml(L.locationsHeader) +
        `</text>`
    );
    locCursorY += 4;
  ...
  const withdrawn = new Set(
    (removedClasses ?? []).map((c) => String(c).toUpperCase())
  );
  ...
  ).filter((k) => !withdrawn.has(String(k).toUpperCase()));
  if (legendClassKeys.length > 0) {
    ...
    if (presentClasses.length > 0) {
      ...
      const PHP_PREFIX = L.currencySymbol;
      const fmtValue = (v) =>
        v == null
          ? ""
      ...
      const roadRows = [
        { tier: "national", label: L.roadNationalLabel },
        { tier: "provincial", label: L.roadProvincialLabel },
        { tier: "barangay", label: L.roadBarangayLabel },
        { tier: "other", label: L.roadOtherLabel },
      ].map((r) => ({
        label: r.label,
      ...
      const innerWidthMm =
        ...
        colGapMm +
        landmarkColWidth;
      const legendWidthMm = Math.max(
        padMm * 2 + innerWidthMm,
        legendBlockWidthMm(
          (targetBarangayFeature
            ? [L.barangayLabel, ...(barangayBorrowedFrom ? ["BOUNDARY SHOWN:"] : [])]
            : []
      ...
      const infoRowsHeightMm =
        ((targetBarangayFeature ? 4 : 3) + (barangayBorrowedFrom ? 1 : 0)) *
        infoLineHeightMm;
      ...
      // Same formula that sized the footer above; see legendBlockHeightMm.
      const legendHeightMm = legendBlockHeightMm({
        commercialCount: commercialRows.length,
        residentialCount: residentialRows.length,
        institutionalCount: institutionalRows.length,
        landmarkCount: landmarkRows.length,
        hasBarangayTarget: Boolean(targetBarangayFeature),
        extraInfoRows: hasBorrowedBoundaryRow ? 1 : 0,
      });
      void infoRowsHeightMm;
      void institutionalHeightMm;
      void colHeightMm;
      ...
      const provinceText = L.provinceValue;
      const islandText = L.islandValue;
      ...
      const infoRows = targetBarangayFeature
        ? [
            [
              ...
              String(
                barangayDisplayName ??
                  targetBarangayFeature.properties?.name ??
                  barangayName ??
                  ""
              ).toUpperCase(),
            ],
            ...(barangayBorrowedFrom
              ? [["BOUNDARY SHOWN:", String(barangayBorrowedFrom).toUpperCase()]]
              : []),
            [L.municipalityLabel, muniText],
            [L.provinceLabel, provinceText],
            [L.islandLabel, islandText],
          ]
        : [
            [L.municipalityLabel, muniText],
            [L.provinceLabel, provinceText],
      ...
      const infoValueX =
        legendX + padMm + infoLabelColumnMm(infoRows.map(([k]) => k));
      ...
      for (const [rowIndex, row] of institutionalRows.entries()) {
        ...
        lines.push(
          `<text x="${fmt(
            ...
            escapeXml(L.institutionalLabel) +
            `</text>`
        );
      }
      ...
      const colLandmarksX = colRoadsX + roadColWidth + colGapMm;
      {
        ...
        const headers = [
          { label: L.commercialHeader, x: colCommercialX, width: smvColWidth },
          { label: L.residentialHeader, x: colResidentialX, width: smvColWidth },
          { label: L.roadsHeader, x: colRoadsX, width: roadColWidth },
          { label: L.landmarksHeader, x: colLandmarksX, width: landmarkColWidth },
        ];
  ...
  // Drawn last so it stays visible above map layers/legend.
  {
    ...
    // Signature block.
    //
    // One to three columns, side by side, each with an optional caption,
    // a name over a signature rule, and a position under it. Columns 2
    // and 3 appear only when named, so the stock one-name sheet is
    // unchanged. This replaces padding a single field with spaces to fake
    // columns, which SVG's whitespace collapsing defeated anyway.
    //
    // Lower-left in both orientations. The landscape right-hand column is
    // only ~130 mm wide; what keeps this off the map is its opaque plate,
    // not its position.
    const signatureColumns = [
      { caption: L.preparedByLabel, name: L.preparedByName, title: L.preparedByTitle },
      { caption: L.preparedByLabel2, name: L.preparedByName2, title: L.preparedByTitle2 },
      { caption: L.preparedByLabel3, name: L.preparedByName3, title: L.preparedByTitle3 },
    ].filter((col, i) => i === 0 || String(col.name ?? "").trim());

    // Bold uppercase runs noticeably wider than regular text, so the name
    // gets a larger factor — at a flat 0.58 a long name overhung its own
    // signature rule.
    const textMm = (text, fontMm, bold = false) =>
      String(text ?? "").length * fontMm * (bold ? 0.68 : 0.58);
    const COL_GAP_MM = 6;
    const colWidths = signatureColumns.map((col) =>
      Math.max(
        38,
        textMm(col.name, 4, true) + 4,
        textMm(col.title, 3.5),
        textMm(col.caption, 3.4)
      )
    );

    const prepX = frameInset + 7;
    const prepY = paperH - frameInset - 25;
    const anyCaption = signatureColumns.some((c) => String(c.caption ?? "").trim());
    const blockW =
      colWidths.reduce((a, b) => a + b, 0) + COL_GAP_MM * (colWidths.length - 1);
    // Never wide enough to leave the sheet or reach the legend column.
    const maxW =
      paperW - prepX - PAGE_MARGIN_MM - 2 -
      (printOrientation === "landscape" ? legendBlockWidthMm() + 6 : 0);
    const plateW = Math.min(Math.max(58, blockW + 8), Math.max(58, maxW));

    lines.push(`<g id="prepared-by" font-family="Helvetica, Arial, sans-serif" fill="#111827">`);
    // Opaque backing, the same treatment the legend gets: roads and
    // neighbouring barangays are painted across the whole sheet, so there
    // is no reliably empty corner to sit in.
    lines.push(
      `<rect x="${fmt(prepX - 3)}" y="${fmt(prepY - 5.5)}" ` +
        `width="${fmt(plateW)}" height="${fmt(25)}" rx="1.2" ` +
        `fill="${PALETTE.paper}" fill-opacity="0.92" ` +
        `stroke="#1f2937" stroke-width="0.25"/>`
    );

    let colX = prepX;
    signatureColumns.forEach((col, i) => {
      const w = colWidths[i];
      const mid = colX + w / 2;
      if (String(col.caption ?? "").trim()) {
        lines.push(
          `<text x="${fmt(colX)}" y="${fmt(prepY)}" font-size="3.4" ` +
            `xml:space="preserve">` +
            escapeXml(col.caption) +
            `</text>`
        );
      }
      if (String(col.name ?? "").trim()) {
        lines.push(
          `<text x="${fmt(mid)}" y="${fmt(prepY + (anyCaption ? 10.5 : 6))}" ` +
      ...
      lines.push(
        `<line x1="${fmt(colX)}" y1="${fmt(prepY + (anyCaption ? 12.3 : 7.8))}" ` +
          `x2="${fmt(colX + w)}" y2="${fmt(prepY + (anyCaption ? 12.3 : 7.8))}" ` +
          `stroke="#111827" stroke-width="0.25"/>`
      );
      if (String(col.title ?? "").trim()) {
        lines.push(
          `<text x="${fmt(mid)}" y="${fmt(prepY + (anyCaption ? 17.2 : 12.7))}" ` +
            `font-size="3.5" text-anchor="middle" xml:space="preserve">` +
            escapeXml(col.title) +
            `</text>`
        );
      }
...
export function buildSvgForSlug(slug, publicDataDir, opts = {}) {
  ...
  // Apply the province-wide palette before rendering. buildSvg is fully
  // synchronous, so no other request can interleave between this and the
  // colorForClass() calls inside it.
  const palette = readPalette(publicDataDir);
  setClassColorOverrides(opts.classColors ?? palette.colors);
  // Overwrite in place — see the note on PALETTE above.
  Object.assign(PALETTE, resolvePrintTheme(opts.theme ?? palette.theme));

  const valuations = readJsonOptional(file(`${slug}_valuations.json`));
  // Three layers, lowest first:
  //   1. the transcribed official schedule in <slug>_valuations.json
  //   2. what an unlocked editor published to <slug>_print_settings.json
  //   3. an unpublished draft passed in by the preview route
  // The valuations file itself is never rewritten, so the official
  // schedule stays readable as the source it is. Reading layer 2 here
  // rather than in the API route means the CLI batch build
  // (scripts/build-print-svg.mjs) produces the same sheet the live
  // /api/print/svg/* routes do.
  const published = readPrintSettings(slug, publicDataDir);
  // Classes this LGU added after transcription sit between the official
  // schedule and any published correction: they are part of the record
  // now, but an explicit override still wins.
  const schedule = getMunicipalityConfig(slug)?.schedule;
  const classEdits = readAddedClasses(slug, publicDataDir, {
    existingSubClasses: (schedule?.classifications ?? []).map((r) => r?.subClass),
    validBarangaySlugs: new Set((schedule?.barangays ?? []).map((b) => b.slug)),
  });
  const classValues = {
    ...extractClassValues(valuations),
    ...addedClassValues(classEdits.classes),
    ...published.classValues,
    ...(opts.classValueOverrides ?? {}),
  };
  // A class the municipality has withdrawn leaves the legend entirely.
  // Zones already drawn in it keep their geometry and still paint, but the
  // sheet stops advertising it as a current tier.
  for (const code of classEdits.removed) delete classValues[code];
  // Which provider landmark kinds to print. Empty/absent means none —
  // the long-standing default. See CIVIC_LANDMARK_KINDS for why this is a
  // filter rather than an on/off switch.
  const landmarkKinds = Array.isArray(opts.landmarkKinds)
    ? opts.landmarkKinds.map((k) => normalizeLandmarkKind(k)).filter(Boolean)
    : [];
  const showOsmPois = landmarkKinds.length > 0;
  const keepKind = new Set(landmarkKinds);
  const onlyKinds = (fc) => {
    ...
    return {
      ...
      features: fc.features.filter((f) =>
        keepKind.has(
          normalizeLandmarkKind(f?.properties?.kind ?? f?.properties?.landmark_kind)
        )
      ),
    };
  };
  ...
  return buildSvg({
    ...
    barangayDisplayName: opts.barangayDisplayName ?? null,
    barangayBorrowedFrom: opts.barangayBorrowedFrom ?? null,
    zoom: opts.zoom ?? 1,
    classLabelScale: opts.classLabelScale ?? 1,
    panX: opts.panX ?? 0,
    panY: opts.panY ?? 0,
    labels: { ...published.labels, ...(opts.labels ?? {}) },
    removedClasses: classEdits.removed,
  });
}
