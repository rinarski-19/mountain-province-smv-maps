...
// Print roads need more visual weight than the screen basemap. Barangay
// sheets get an additional bump because their larger geographic scale makes
// the fixed paper-width strokes look disproportionately light.
const PRINT_ROAD_WIDTH_SCALE = 2.4;
const BARANGAY_ROAD_WIDTH_SCALE = 1.3;
...
// Candidate points for a zone label. pointOnFeature is usually ideal, but
// an overlapping SMV polygon can cover that exact point later in the paint
// order. The centroid and a small deterministic bbox grid give the labeler
// alternate interior points without introducing random output.
...
function visibleZoneLabelPoint(entry, drawIndex, entries) {
  for (const point of zoneLabelPointCandidates(entry.feat)) {
    try {
      ...
      if (!coveredByDifferentClass) return point;
    } catch {}
  }
  return null;
}
...
// Print intentionally uses category colors rather than value-tier colors:
// every commercial class reads as the C-1 color and every residential class
// reads as the R-1 color. The class labels and values remain in the legend.
function printColorForClass(klass, slug) {
  if (isCommercialClass(klass)) return colorForClass("C-1", slug);
  if (isResidentialClass(klass)) return colorForClass("R-1", slug);
...
export function buildSvg({
  ...
  places,
  landmarks,
  // Keep print geometry identical to the live map by default. A positive
  // value remains available for callers that explicitly want schematic
  // widening, but the normal print route must not alter saved zone shapes.
  smvBufferM = 0,
  classValues = {},
  ...
  //
  // smvBufferM > 0 optionally widens ribbon-shaped zones (compactness
  // < 0.15) at render time. The normal value is zero so the SVG uses
  // the exact saved GeoJSON shape shown by the live map.
  ...
  if (zoneFeats.length) {
    ...
    for (const { feat: f } of sorted) {
      ...
      // polygon.
      let renderFeat = f;
      if (smvBufferM > 0 && ribbonCompactness(f) < RIBBON_COMPACTNESS) {
  ...
  if (roadFeats.length) {
    ...
    for (const tier of TIER_PAINT_ORDER) {
      ...
      for (const f of feats) {
        ...
        const base =
          (ROAD_WEIGHTS_MM[hw] ?? ROAD_DEFAULT_MM) * roadWidthScale;
  ...
  if (zoneLabelEntries.length) {
    ...
    for (const { feat, klass, bounds, drawIndex } of labelCandidates) {
      try {
        const point = visibleZoneLabelPoint(
          { feat, klass },
          drawIndex,
          zoneLabelEntries
        );
        ...
        // Fit the text to the zone at the current export scale. Tiny zones
    ...
    for (const f of landmarkFeats) {
      ...
      lines.push(
        `<g transform="translate(${cx} ${cy})">` +
          ...
          `<path d="${iconPath}" transform="translate(-1.5 -1.5) scale(0.125)" ` +
