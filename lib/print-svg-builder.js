...
export function buildSvg({
  ...
  // Drawn last so it stays visible above map layers/legend.
  {
    ...
    // Prepared-by block.
    //
    // Portrait keeps it lower-left, where the reserved footer leaves room.
    // Landscape now gives the map the full sheet height, so lower-left is
    // live map — the signature printed straight over barangay boundaries
    // and SMV bands. Put it in the right-hand column instead, in the gap
    // between compass and legend that was otherwise blank.
    // Lower-left in both orientations. The landscape right-hand column is
    // only ~130 mm wide, which cannot hold a two-signatory block; the
    // bottom-left strip has ~280 mm before the legend column starts. The
    // opaque plate below is what keeps it off the map, not its position.
    ...
    // neighbouring barangays are painted across the whole sheet, not just
    // inside the map frame, so there is no reliably empty corner to sit
    // in — on landscape the signature printed straight through boundary
    // lines and SMV bands. Rather than hunt for white space, carry it.
    // Sized to the actual signature, not a constant: LGUs line up two
    // signatories with spaces, so this block can be far wider than the
    // stock one name. A fixed 58 mm left that text hanging outside its
    // own backing, over live map.
    const prepTextMm = (text, fontMm) =>
      String(text ?? "").length * fontMm * 0.58;
    // Never wider than the space before the legend column, or the plate
    // itself runs off the sheet.
    const prepMaxWidthMm =
      paperW - prepX - PAGE_MARGIN_MM - 2 -
      (printOrientation === "landscape" ? legendBlockWidthMm() + 6 : 0);
    const prepWidthMm = Math.min(
      Math.max(
        58,
        prepTextMm(L.preparedByName, 4) + 16,
        prepTextMm(L.preparedByTitle, 3.5) + 24,
        prepTextMm(L.preparedByLabel, 3.4) + 6
      ),
      Math.max(58, prepMaxWidthMm)
    );
    lines.push(
      `<rect x="${fmt(prepX - 3)}" y="${fmt(prepY - 5.5)}" ` +
        `width="${fmt(prepWidthMm)}" height="${fmt(25)}" rx="1.2" ` +
        `fill="${PALETTE.paper}" fill-opacity="0.92" ` +
        `stroke="#1f2937" stroke-width="0.25"/>`
    );
    lines.push(
      `<text x="${fmt(prepX)}" y="${fmt(prepY)}" font-size="3.4">` +
        escapeXml(L.preparedByLabel) +
        `</text>`
    );
    lines.push(
      `<text x="${fmt(prepX + 13)}" y="${fmt(prepY + 10.5)}" ` +
        `font-size="4" font-weight="700" xml:space="preserve">` +
        escapeXml(L.preparedByName) +
        `</text>`
    );
    lines.push(
      `<line x1="${fmt(prepX + 13)}" y1="${fmt(prepY + 12.3)}" ` +
        `x2="${fmt(prepX + 13 + Math.max(35, prepTextMm(L.preparedByName, 4)))}" ` +
        `y2="${fmt(prepY + 12.3)}" ` +
        `stroke="#111827" stroke-width="0.25"/>`
    );
    lines.push(
      `<text x="${fmt(prepX + 21.2)}" y="${fmt(prepY + 17.2)}" ` +
        `font-size="3.5" xml:space="preserve">` +
        escapeXml(L.preparedByTitle) +
        `</text>`
    );
    lines.push(`</g>`);
