...
// 30 m per side across every road tier so the SMV color band is the
...
// 2 × 30 m = 90 m. If specific tiers ever need to differ again
...
export function buildSvg({
  ...
  if (zoneFeats.length) {
    ...
    for (const { feat: f } of sorted) {
      ...
      // meters per side. The buffer size is TIER-AWARE — N comes from
      // TIER_BUFFER_M based on which road tier this polygon hugs:
      //   national  → +30 m  (SMV halo competes with the wide highway)
      //   provincial → +25 m
      //   barangay  → +15 m
      //   other     → +12 m
      //   no nearby road within TIER_BUFFER_MAX_DIST_M → smvBufferM
    ...
    lines.push(`</g>`);
    zonesSummary = { rendered: sorted.length, byClass, widened, smvBufferM };
  }
  ...
  // Only SMV classes that actually appear in zonesSummary.byClass show
  // up; roads always show all four tiers since the basemap uses them.
  // Values come from <slug>_valuations.json via classValues.
  if (zonesSummary?.byClass && Object.keys(zonesSummary.byClass).length > 0) {
    const presentClasses = Object.keys(zonesSummary.byClass)
...
export function buildSvgForSlug(slug, publicDataDir, opts = {}) {
  ...
  return buildSvg({
    ...
    roads: readJsonOptional(file(`${slug}_osm_roads.geojson`)),
