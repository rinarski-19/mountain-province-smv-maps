...
export function buildSvg({
  ...
  // Drawn last so it stays visible above map layers/legend.
  {
    ...
    const prepInColumn =
      printOrientation === "landscape" && Boolean(targetBarangayFeature);
    const prepX = prepInColumn
      ? paperW - PAGE_MARGIN_MM - legendBlockWidthMm() + 4
      : frameInset + 7;
    const prepY = prepInColumn
      ? paperH - PAGE_MARGIN_MM - reservedLegendHeightMm - 30
      : paperH - frameInset - 25;
    ...
    lines.push(
      `<rect x="${fmt(prepX - 3)}" y="${fmt(prepY - 5.5)}" ` +
        `width="${fmt(58)}" height="${fmt(25)}" rx="1.2" ` +
