...
const BARANGAY_CROP_MARGIN_M = 250;
...
// TEST (2026-07-14): which classes render via the cast-iron hatch
// pattern instead of / on top of a solid fill — as a primary class
// (replaces the solid fill entirely) or as a secondary/tertiary class
// (drawn as an overlay on top of the primary's solid fill). One toggle
// controls both cases. Revert by emptying this set.
const HATCH_TEST_CLASSES = new Set(["C-1", "C-2"]);
...
export function buildSvg({
  ...
  if (targetBarangayFeature) {
    ...
    renderPlaces = featureCollectionNearBbox(places, filterBbox);
    renderLandmarks = featureCollectionNearBbox(landmarks, filterBbox);
  ...
  for (const klass of ["C-1", "C-2", "C-3", "C-4"]) {
    ...
    const hatchColor = commercialHatchColorForClass(klass, slug);
    const lineAttrs =
  ...
  if (zoneFeats.length) {
    ...
    const _barangayFeatsForBufferLookup = renderBarangays?.features ?? [];
    ...
    // Collected during the loop below, then joined-by-class and drawn
    // after it (see joinZonesByClass call further down).
    ...
    // Join same-class polygons (joinZonesByClass, defined above) before
    // drawing: adjacent/overlapping buffered ribbons of the same class
    // now share one clean outer boundary instead of each drawing its own
    // stroke, which used to stack into a visibly darker seam where they
    // touch. Z-order is by class value (lower underneath, higher on top)
    // — same policy the old per-feature sort used, just applied once per
    // class now that same-class polygons are merged into one shape.
    const parcelEntries = primaryEntries.filter(({ originalFeat }) => {
      const props = originalFeat?.properties ?? {};
      return (
        props.source === "dxf" ||
        props.parcel_id != null ||
        props.lot_number != null ||
        props.lot_no != null ||
        // Older editor-drawn Catengan parcels predate explicit source
        // metadata. Treat reasonably small hand-drawn polygons as parcel
        // portions, but keep generated road corridors eligible for joining.
        (props.source !== "road-tag" && featureArea(originalFeat) <= 25000)
      );
    });
    const parcelEntrySet = new Set(parcelEntries);
    const corridorEntries = primaryEntries.filter(
      (entry) => !parcelEntrySet.has(entry)
    );
    const joinedByClass = joinZonesByClass(
      corridorEntries.map((e) => ({ feat: e.renderFeat, klass: e.solidClass }))
    ).sort((a, b) => {
      const av = CLASSIFICATION_INFO[a.klass]?.value ?? -Infinity;
      const bv = CLASSIFICATION_INFO[b.klass]?.value ?? -Infinity;
      return av - bv;
    });
    for (const { feat, klass } of joinedByClass) {
      const joinedD = geometryToPathD(feat.geometry, project);
      if (!joinedD) continue;
      const fill = HATCH_TEST_CLASSES.has(klass)
        ? `url(#${commercialHatchPatternId(klass)})`
        : getZoneColor(klass, slug);
      lines.push(
        `<path d="${joinedD}" fill="${fill}" stroke="#000000" stroke-width="0.15" ` +
          `data-class="${escapeXml(klass)}"/>`
      );
    }

    // Cadastral/DXF polygons must remain separate even when neighboring lots
    // share a class. Their individual black boundaries are the foundation of
    // a parcel-level SMV map; joining by class would erase those lot lines.
    for (const { renderFeat, originalFeat, solidClass, d } of parcelEntries) {
      const fill = HATCH_TEST_CLASSES.has(solidClass)
        ? `url(#${commercialHatchPatternId(solidClass)})`
        : getZoneColor(solidClass, slug);
      lines.push(
        `<path d="${d}" fill="${fill}" stroke="#222222" stroke-width="0.18" ` +
          `data-class="${escapeXml(solidClass)}" data-render="parcel"/>`
      );
      parcelLabelEntries.push({ feat: originalFeat ?? renderFeat, klass: solidClass });
    }
  ...
  // Parcel SMV labels. Use pointOnFeature rather than a raw centroid so the
  // label remains inside concave and irregular cadastral lots.
  if (parcelLabelEntries.length) {
    lines.push(
      `<g id="parcel-class-labels" font-family="Arial, Helvetica, sans-serif" ` +
        `text-anchor="middle" dominant-baseline="middle" font-weight="700" ` +
        `fill="#171717">`
    );
    const labelCandidates = parcelLabelEntries
      .map(({ feat, klass }) => {
        const bounds = geometryProjectedBounds(feat?.geometry, project);
    ...
    for (const { feat, klass, bounds } of labelCandidates) {
      try {
        const point = turf.pointOnFeature(feat);
        const [lon, lat] = point.geometry.coordinates;
        const [x, y] = project(lon, lat);
        const width = bounds.maxX - bounds.minX;
        const height = bounds.maxY - bounds.minY;
        // Fit the text to the parcel at the current export scale. Tiny lots
        // are left unlabeled on whole-LGU plates instead of being obscured;
        // they become eligible automatically in a closer barangay export.
        const fontSize = Math.min(
          1.1,
          width / Math.max(1, klass.length * 0.72),
          height * 0.42
        );
        if (fontSize < 0.42) continue;
        const textWidth = fontSize * klass.length * 0.62;
        ...
        const overlaps = occupiedLabelBoxes.some(
          (other) =>
            box.minX < other.maxX &&
            box.maxX > other.minX &&
            box.minY < other.maxY &&
        ...
        lines.push(
          `<text x="${fmt(x)}" y="${fmt(y)}" font-size="${fontSize.toFixed(2)}" ` +
            `stroke="#ffffff" stroke-width="${Math.max(0.12, fontSize * 0.18).toFixed(2)}" ` +
            `paint-order="stroke" stroke-linejoin="round">${escapeXml(klass)}</text>`
        );
      } catch {}
    }
    lines.push(`</g>`);
  ...
  // Landmark pins — rendered on top of SMV zones, roads, boundaries,
  // and place labels. These are saved OSM/custom landmark features,
  // not provider-baked POIs, so the print is deterministic.
  ...
  if (landmarkFeats.length) {
    ...
    for (const f of landmarkFeats) {
      ...
      const symbol = landmarkSymbol(kind);
      const cx = fmt(x);
      const cy = fmt(y);
      const label = escapeXml(name);
      ...
      lines.push(
        `<g transform="translate(${cx} ${cy})">` +
          `<rect x="-2.2" y="-2.2" width="4.4" height="4.4" rx="2.2" ` +
          `transform="rotate(-45)" fill="#7d8a8d" stroke="#ffffff" ` +
          `stroke-width="0.72"/>` +
          `<rect x="-2.2" y="-2.2" width="4.4" height="4.4" rx="2.2" ` +
          `transform="rotate(-45)" fill="none" stroke="#1f2937" ` +
          `stroke-width="0.16" opacity="0.55"/>` +
          `<text x="0" y="0.72" text-anchor="middle" font-size="2.8" ` +
          `fill="#ffffff" font-family="Georgia, 'Times New Roman', serif" ` +
          `font-weight="900">${sym}</text>` +
          `</g>`
      );
      const labelX = x + 3.4;
      const labelY = y + 1.0;
      ...
      lines.push(
        `<rect x="${fmt(labelX - 0.8)}" y="${fmt(labelY - 3.0)}" ` +
          `width="${fmt(Math.max(10, String(name).length * 1.55 + 1.6))}" ` +
          `height="4.1" rx="0.9" fill="#ffffff" fill-opacity="0.94" ` +
          `stroke="#1f2937" stroke-opacity="0.22" stroke-width="0.12"/>`
      );
      lines.push(
        `<text x="${fmt(labelX)}" y="${fmt(labelY)}" font-size="2.5" ` +
