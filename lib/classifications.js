...
// Hand-picked once the yellow→amber ramp runs out of usable contrast.
// The old clamp at 0.52 was reached at R-8, so R-8..R-15 all rendered
// #FFC300 — on Sagada's and Tadian's deep residential ladders that made
// eight different value classes indistinguishable on screen and on paper.
// These stay warm (so the residential family still reads as a family) and
// clear of the blue/cyan band reserved for water.
const RESIDENTIAL_PALETTE_TAIL = {
  8: "#F4A300", // amber
  9: "#E08700", // dark amber
  10: "#C86F00", // ochre
  11: "#A85A0B", // tan-brown
  12: "#8C4A14", // sienna
  13: "#6F3A16", // dark brown
  14: "#B08968", // muted clay
  15: "#8D6E4A", // stone brown
};

const residentialShade = (number) =>
  RESIDENTIAL_PALETTE_TAIL[number] ??
  mixCategoryColor(
...
// Editable palette.
//
// An unlocked editor can recolour any SMV class from the print panel.
// The override is province-wide, not per-LGU: C-1 has to mean the same
// red in Bauko as in Sagada or sheets from different municipalities stop
// being comparable.
//
// Held as module state consulted by colorForClass() — the one resolver
// every surface goes through — so a change reaches the map polygons, the
// sidebar chips, the hover card and the printed sheet from a single
// assignment, without re-plumbing all of them.
//
// On the server this is set immediately before buildSvg(), which is
// fully synchronous, so a request cannot interleave with another's
// palette. On the client it is set once the overrides are fetched.
// ---------------------------------------------------------------------
export const DEFAULT_CLASS_COLORS = CLASSIFICATION_COLORS;

let CLASS_COLOR_OVERRIDES = {};

const HEX_COLOR = /^#[0-9a-fA-F]{6}$/;

// Keep only well-formed hex values for classes that actually exist —
// anything else would paint an invalid fill and silently blank a zone.
export function sanitizeClassColors(colors) {
  const out = {};
  if (!colors || typeof colors !== "object") return out;
  for (const [klass, value] of Object.entries(colors)) {
    const key = String(klass).trim().toUpperCase();
    if (!CLASSIFICATION_INFO[key]) continue;
    if (typeof value !== "string") continue;
    const hex = value.trim().toLowerCase();
    if (!HEX_COLOR.test(hex)) continue;
    if (hex === String(CLASSIFICATION_INFO[key].color).toLowerCase()) continue;
    out[key] = hex;
  }
  return out;
}

export function setClassColorOverrides(colors) {
  CLASS_COLOR_OVERRIDES = sanitizeClassColors(colors);
  return CLASS_COLOR_OVERRIDES;
}

export function getClassColorOverrides() {
  return { ...CLASS_COLOR_OVERRIDES };
}

// Resolve the fill / stroke color from the shared category palette. The
// optional slug remains accepted for API compatibility, but colors are
// intentionally consistent across every municipality and output surface.
export function colorForClass(klass, slug = null) {
  const key = String(klass ?? "").trim().toUpperCase();
  return (
    CLASS_COLOR_OVERRIDES[key] ??
